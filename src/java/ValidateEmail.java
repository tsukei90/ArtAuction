        ...
        try (PrintWriter out = response.getWriter()) {
           ...
           ost =(OraclePreparedStatement) oconn.prepareStatement("SELECT * FROM USERS where email=?");
            ...
            try {
                    ...
                    vbody = "Please click the email below or copy paste it to reset your password\n";
                    vbody += "http://localhost:8080/WebApplication_TEST/ForgotPass/VerifyQues.jsp?pemail=" + vemail;
                    message.setText(vbody);
                    Transport.send(message);
                    response.sendRedirect("http://localhost:8080/WebApplication_TEST/ForgotPass/ForgotClose.html");
            ...
            {
                response.sendRedirect("http://localhost:8080/WebApplication_TEST/ForgotPass/WrongEmail.html");
            }
            ost.close();
            oconn.close();
            out.println("</body>");
